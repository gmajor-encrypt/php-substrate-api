...
use Rpc\Contract\InstantiateRequest;
...
class Contract
{
    ...
    public function new (string $code, mixed $data, array $option = []): InstantiateRequest
    {
        ...
        // estimate gasLimit
        $instantiateRequest = ContractExecResult::deserialization($this->instantiateRequest($code, $data, $salt));
        $gasLimit = array_key_exists("gasLimit", $option) ? ["proof_size" => 0, "ref_time" => $option["gasLimit"]] : ContractExecResult::convertGasRequired($instantiateRequest->gasRequired);
        $storageDepositLimit = array_key_exists("storageDepositLimit", $option) ? $option["storageDepositLimit"] : 0;


        return new InstantiateRequest($instantiateRequest->result->Ok->accountId, $this->tx->Contracts->instantiate_with_code($storageDepositLimit, $gasLimit, null, Utils::hexToBytes($code), Utils::hexToBytes($data), Utils::hexToBytes($salt)));
    }
    ...
    public function instantiateRequest (string $code, string $input, string $salt): mixed
    {
        ...
        $data = $data . $codec->createTypeByTypeString("bytes")->encode($code); // code
        $data = $data . $codec->createTypeByTypeString("bytes")->encode($input); // input
